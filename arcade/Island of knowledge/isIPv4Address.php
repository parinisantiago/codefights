...
{
    if(preg_match("/[a-z]/i", $inputString)) return false;
    
    $ipv4 = explode('.',$inputString);
    if(sizeOf($ipv4) != 4) return false;
   
    foreach($ipv4 as $pivot)
    {
        if ($pivot == null) return false;
        if ($pivot < 0 || $pivot > 255) return false;
    }   
